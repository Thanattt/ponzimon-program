...
// Initial registration reward: one Rare card, matching the public docs.
pub const STARTER_CARD_IDS: [u16; 1] = [42]; // Gustgoat
...
pub const SEASON_DURATION_SLOTS: u64 = 30 * SLOTS_PER_DAY;
pub const DAILY_EMISSIONS_TOKENS: u64 = 800_000;
// 800,000 * 1,000,000 / 216,000 = 3,703,703.703...
pub const SEASON_REWARD_RATE: u64 = 3_703_704;
...
pub fn get_card_by_id(id: u16) -> Option<(u8, u16, u8)> {
    CARD_DATA
        ...
        .map(|(_, rarity, _, _)| {
            let hashpower = 4u16.pow((*rarity as u32) + 1);
            (*rarity, hashpower, 0)
        })
}
